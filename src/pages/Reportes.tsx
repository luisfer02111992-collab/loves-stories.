...
export default function Reportes(){
 ...
 return <div>
  ...
  {periodo==='rango'&&<div className="flex flex-wrap items-end gap-3 mb-4 p-3 rounded-lg bg-white border"><label className="text-xs">Desde<input type="date" value={desde} onChange={e=>setDesde(e.target.value)} className="block mt-1 px-3 py-2 rounded border"/></label><label className="text-xs">Hasta<input type="date" value={hasta} min={desde} onChange={e=>setHasta(e.target.value)} className="block mt-1 px-3 py-2 rounded border"/></label><button disabled={cargando||!desde||!hasta||hasta<desde} onClick={()=>{setRangoDesde(desde);setRangoHasta(hasta)}} className="px-4 py-2 rounded-md text-sm flex items-center gap-2 text-white disabled:opacity-50" style={{background:color1}}><Search size={15}/>{cargando?'Buscando…':'Buscar'}</button><span className="text-xs text-gray-500 pb-2">Mostrando: {tituloPeriodo(periodo,rangoDesde,rangoHasta)}</span></div>}
